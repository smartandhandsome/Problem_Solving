...
낮은 나무는 잘리지 않을 것이다. 예를 들어, 한 줄에 연속해있는 나무의 높이가 20, 15, 10, 17이라고 하자.
...
while MIN <= MAX:
    ...
    leng = sum([t - MID if t > MID else 0 for t in tree])
    """
    for t in tree
        if t - MID > 0:
        leng += (t - MID)
...
"""
파이썬 코드는 함수를 바이트코드로 컴파일하기 때문에 함수로 실행 했을 시 더 빠르다.
참고: https://stackoverflow.com/questions/11241523/why-does-python-code-run-faster-in-a-function
"""